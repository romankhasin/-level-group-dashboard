...
def main():
    ...
    meta=fast.preclassify_meta(fast.base.load_meta(token,project_id)); projects={}; unknown=0; total_imps=0; total_with=0
      ...
      with gzip.GzipFile(fileobj=response) as compressed:
       with io.TextIOWrapper(compressed,encoding="utf-8-sig",newline="") as stream:
        for row in csv.DictReader(stream):
            ...
            if item: ch=item["channel"]; pname=item["project"]; scope=item["scope"]
    ...
    data={"from":a.from_date,"to":a.to_date,"jobId":job,"projectId":project_id,"placementMetaCount":len(meta),"totalImpressions":total_imps,"totalWithDevice":total_with,"unknownPlacementImpressions":unknown,"projects":projects}
