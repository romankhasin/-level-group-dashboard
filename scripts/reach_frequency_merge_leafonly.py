...
def main():
    ...
    result={"period":{"from":a.from_date,"to":a.to_date},"projectId":project_id,"method":"Target Ads Raw Data API v2; parallel date chunks; exact 64-bit hashed InteractionDeviceID Roaring Bitmap unions","important":"Reach is deduplicated independently for Total, each channel, each object and each object×channel pair. Do not sum child Reach rows.","channelClassificationVersion":"v1-2026-08-17","projectClassificationVersion":"v1-2026-08-17","channelClassificationNotes":{"Programmatic":"Classified only when the confirmed prg token is present in Target Ads naming","Медийка":"Classified only when the confirmed med token is present in Target Ads naming","Маркетплейсы":"Classified only when the confirmed mrk token is present in Target Ads naming"},"projectClassificationNotes":"Known Level development names are normalized; regional placements roll into the same object. Brand and unknown prefixes remain separate.","placementMetaCount":meta_count,"total":{"label":"Level Group","impressions":total_imps,"impressionsWithDevice":total_with,"reach":total_reach,"frequency":round(total_imps/total_reach,4) if total_reach else None,"deviceIdCoverage":round(total_with/total_imps,6) if total_imps else None},"byChannel":by_channel,"byProject":rows,"unclassified":unclassified,"unknownPlacementImpressions":unknown,"jobs":sorted(jobs,key=lambda x:x["from"])}
